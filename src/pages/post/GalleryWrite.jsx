...
export default function GalleryWrite() {
    ...
    // 토글 스위치 및 입력 상태 관리
    const [toggles, setToggles] = useState({ comment: false, share: false });
    ...
    const handleSubmit = () => {
        ...
        const savedPosts = JSON.parse(localStorage.getItem('gallery_posts')) || [];
                ...
                <div className="gw-content">
                    ...
                    <div className="gw-right-form">
                        <div className="form-group">
                            <label>제목</label>
                            <input type="text" className="gray-input" placeholder="제목추가" value={title} onChange={(e) => setTitle(e.target.value)} />
                        </div>
                        <div className="form-group">
                            <label>설명</label>
                            <textarea className="gray-textarea" placeholder="자세한 설명을 추가하세요" value={description} onChange={(e) => setDescription(e.target.value)}></textarea>
                        </div>

...
export default function MyPage() {
    ...
    const [infoTab, setInfoTab] = useState('account');
    ...
    const [userName, setUserName] = useState('숲속으로'); // 상단에 보여질 닉네임(또는 이름)
    const [bio, setBio] = useState('한줄소개쓰는 공간');
    const [isEditingBio, setIsEditingBio] = useState(false);

    // 닉네임 변경 관련 상태
    const [editNickname, setEditNickname] = useState('');
    const [isNicknameChecked, setIsNicknameChecked] = useState(true); // 처음엔 기존 닉네임이니 true
    ...
    useEffect(() => {
        ...
        if (savedUserString) {
            ...
            currentDisplayName = savedUser.nickname || savedUser.name;
        ...
        // 2. 내가 쓴 글 불러오기
        const commPosts = JSON.parse(localStorage.getItem('community_posts')) || [];
        const forumPosts = JSON.parse(localStorage.getItem('forum_posts')) || [];
        const allMyPosts = [
            ...commPosts.filter(p => p.author === currentDisplayName).map(p => ({ ...p, type: 'community' })),
            ...forumPosts.filter(p => p.author === currentDisplayName).map(p => ({ ...p, type: 'forum' }))
        ].sort((a, b) => b.id - a.id);
        setMyPosts(allMyPosts);

        // 3. 내가 쓴 댓글 불러오기
        const commComments = JSON.parse(localStorage.getItem('community_comments')) || [];
        const forumComments = JSON.parse(localStorage.getItem('forum_comments')) || [];
    ...
    // 한줄소개 저장
    const handleSaveBio = () => {
        const savedUser = { ...userData, bio: bio };
    ...
    const handleSaveAccount = () => {
        ...
        const updatedUser = { ...userData, nickname: editNickname };
        ...
        if (infoTab === 'social') {
            return (
                <div className="info-content">
                    ...
                    <div className="social-connect-item">
                        <div className="social-info"><div className="social-icon" style={{backgroundColor: '#ea4335'}}>G</div><span>연동되지 않음</span></div>
                        <button className="connect-btn active">연동하기</button>
                    </div>
                    <div className="social-connect-item">
                        <div className="social-info"><div className="social-icon" style={{backgroundColor: '#03c75a'}}>N</div><span>연동되지 않음</span></div>
                        <button className="connect-btn active">연동하기</button>
                    </div>
                    <div className="social-connect-item">
                        <div className="social-info"><div className="social-icon" style={{backgroundColor: '#000'}}></div><span>연동되지 않음</span></div>
                        <button className="connect-btn active">연동하기</button>
                    </div>
                </div>
            );
                ...
                <div className="profile-info">
                    ...
                    {isEditingBio ? (
                        <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                            <input type="text" value={bio} onChange={(e) => setBio(e.target.value)} style={{ padding: '6px', borderRadius: '4px', border: '1px solid #ccc', width: '200px' }} autoFocus />
